...
const Questions = ({ questions, filterData }) => {
  return (
    <>
      <div className="w-full h-[550px] flex flex-col mt-6 overflow-y-scroll">
        {questions.length == 0 ? (
          ...
          <div>
            {questions.map((q, index) => {
              return (
                ...
                >
                  <div className="flex items-start sm:items-center gap-2 px-4 py-2 md:py-3 sm:p-2 text-sm ">
                    <span className="sm:px-4">
                      <img src={CheckImg} alt="check-img" className="w-3 h-3 mt-1 sm:mt-0" />
                    </span>
                    <span>{q.q_id}</span>
